...
# data path
path='/home/ehsan/hadi/new_reduced_size'
...
## CCA benchmark
Freq_phase = sio.loadmat('/home/ehsan/hadi/Freq_Phase')
...
train_data=CCA_features[0][:,:,train_index]
test_data=CCA_features[0][:,:,test_index]

train_data=np.transpose(train_data,(2,1,0))
test_data=np.transpose(test_data,(2,1,0))

# save CCA_features and Acc
np.savez_compressed('CCA_train', CCA_features=train_data)

# save CCA_features and Acc
np.savez_compressed('CCA_test', CCA_features=test_data)
...
LearningRate = 0.001
workspace = '/home/ehsan/hadi/net2'
PathData = '/home/ehsan/hadi/CCA_train.npz'
PathData_test = '/home/ehsan/hadi/CCA_test.npz'
Batch_size = [128] 
Epochs = 25
#data
n_channels=64 # all channels
...
for N_Neuron in N_Neurons_GRU:
    for bs in Batch_size:
        ...
        ##structure
        x_shape_eeg,y_shape_eeg = 250,40
